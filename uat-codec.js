...
(() => {
  ...
  const minifyHtml = (html) => {
    ...
    // unquoted 値の char class から `/` を除外する点が極めて重要 (inner re と一致させる)。
    //   除外しないと、直前の引用符除去で `<line ... y2="180"/>` → `<line ... y2=180/>` に
    //   なった自己終了タグの outer マッチで、unquoted 値が `180/` まで貪欲に食い進み、
    //   自己終了スラッシュ `/` を吸い込んで slash キャプチャ群が空になる。直後の inner re は
    //   `/` を除外しているので `y2=180` だけが拾われ、再構築時に slash 群が空のまま
    //   `<line ... y2=180>` として出力される (= 自己終了が永久に失われる)。
    //   結果、SVG foreign content 内の `<line>` `<circle>` `<rect>` `<polygon>` `<path>` 等の
    //   自己終了タグが後続要素を子要素として呑み込むパースになり、グラフ・図形・
    //   アニメーションが軒並み描画されない症状が出ていた (Copy Short URL = minify 必須経路
    //   でだけ再現、生 HTML 直貼りでは minify を通らないため正常)。
    //   なお下流の `(=value)/>` → `(=value) />` フィックスは「outer が slash を保存できた後」の
    //   HTML5 仕様上の安全装置として依然必要 (unquoted attribute value state で `/` が値の
    //   一部として吸われる挙動を確実に終端させる)。
